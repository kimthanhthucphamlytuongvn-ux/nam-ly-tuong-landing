...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
    >
      <GlassCard className="flex h-full flex-col overflow-hidden transition-shadow duration-300 hover:shadow-glow-green">
        <div className="flex aspect-[4/3] items-center justify-center border-b border-glass-border bg-bg-1 [background-image:radial-gradient(120%_120%_at_30%_20%,rgba(51,230,160,0.14),transparent_60%)]">
          <Icon className="h-[44%] w-[44%] text-neon-gold opacity-90" />
        </div>
        <div className="flex flex-1 flex-col gap-1.5 p-[18px] pb-5">
          <span className="font-body text-[0.68rem] font-bold uppercase tracking-[0.1em] text-neon-green">
